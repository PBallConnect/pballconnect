...
| `matches` | Match events | `id`, `organizer_email`, `match_date`, `time_start`, `time_end`, `court_id`, `court_name`, `num_courts`, `gender_pref`, `max_players` |
| `match_responses` | Per-player responses | `match_id`, `player_email`, `player_name`, `response` ('in'/'out'/'pending'/'waitlist'/'waitlist_left' — `match_responses_response_check` was explicitly altered in Supabase on July 3, 2026 to add `'waitlist_left'` as a confirmed, live-allowed value: `CHECK (response = ANY (ARRAY['pending','in','out','waitlist','waitlist_left']))`; written by `leaveWaitlist()` when a player voluntarily leaves a `'waitlist'`-status row), `responded_at`, `filled_from_waitlist` (boolean — true only when a `'pending'` row was created by a genuine waitlist promotion; see CLAUDE-RULES.md Rule 60) |
...
| `action_tokens` | Opaque short-token store backing the match-invite and waitlist-promo signed-link redesign (added July 2026 — replaces the old HMAC-signed long-URL pattern, which was confirmed to get corrupted in transit over real SMS delivery) | `token` (text, PK — short random URL-safe string, e.g. `_iKOQmehS3NN`), `link_type` (text — CHECK constraint `IN ('match_invite','waitlist_promo','match_view')`), `payload` (jsonb — arbitrary per-link data resolved server-side, e.g. `{matchId, inviteePhone, inviteeName, organizerEmail}` for `match_invite`, `{matchId, playerEmail}` for `waitlist_promo`, `{matchId}` for `match_view`), `expiry` (timestamptz), `used_at` (timestamptz, nullable — best-effort observability marker only, never gates re-visiting a still-valid link), `created_at` (timestamptz, default `now()`) |
| `bug_tracker` | Quick-glance status view for the numbered bugs tracked in CLAUDE-TESTING.md (added Aug 2026, populated with all bugs known at creation time) — NOT the detailed record, just current status per bug | `id` (matches the Bug N number in CLAUDE-TESTING.md), `title`, `status` ('Open'/'In Progress'/'Complete'), `priority`, `description`, `found_date`, `last_updated` |
...
> **RLS (`action_tokens`):** Enabled with zero policies — service-role only, same posture as `waitlist`/`beta_applications`. The only code that reads/writes this table is `functions/_shared/action-tokens.js` (`createActionToken`, `resolveActionToken`, `markActionTokenUsed`). See CLAUDE-FLOWS.md Flow 9 for the updated match-invite mechanism this replaces, and CLAUDE-SMS.md for the historical HMAC-signed design it superseded.

> **⚠️ KNOWN GAP — no committed DDL:** `action_tokens` has no `CREATE TABLE` statement tracked anywhere in this repo — not in this file as real SQL, not in `supabase_rls_policies.sql` (confirmed empty for this table). The table was created directly in Supabase's SQL editor and was never backfilled into version control. Treat the live Supabase schema as authoritative for this table until this gap is closed.

> **No admin/staff role concept exists anywhere in this codebase** (confirmed via a repo-wide grep, July 2026 RLS audit). `is_organizer` is reporting-only, not an access-control mechanism — see the June 2026 note under Organizer Nav Gating below. There is currently no column, flag, or check anywhere distinguishing "regular player" from "someone allowed to see everyone's data."
...
> **`bug_tracker` / CLAUDE-TESTING.md sync convention (established Aug 2026):** whenever a bug is newly found, INSERT a row into `bug_tracker`; whenever a bug's status changes (Open -> In Progress -> Complete), UPDATE the row and set `last_updated = now()`. CLAUDE-TESTING.md remains the detailed technical record (root cause, code traces, line numbers, live-verification notes) — `bug_tracker` is the quick-glance status view, not a replacement. Keep both in sync on every status change, in the same pass, not deferred. Claude Code has no direct SQL execution access to Supabase (see the RLS-audit notes throughout CLAUDE-TESTING.md for the same limitation) — INSERT/UPDATE statements must be handed to the user to run in the Supabase SQL editor, same as every other live database change.

> **`connections` and `match_responses` have no FK/cascade relationship** (confirmed by code trace, Aug 5/6 2026). A match roster reflects IC membership at the time of invite/accept, not current IC status — if an IC `connections` row is later removed, any existing `match_responses` rows for that pair are left untouched. This is expected behavior, not a bug: traced and confirmed via a real match where a confirmed participant ("zorro") was no longer present in the organizer's current IC list. See CLAUDE-TESTING.md for the full trace (organizer-side invite grid and Emergency Fill are both IC-gated with no live path to add a non-IC player today — Known Bug #3 — so the roster mismatch reflects a since-removed IC connection, not a bypass of that gating).

> **⚠️ SCHEMA AUDIT WARNING (confirmed July 2026):** The `matches` row above previously listed a `format` column that does NOT exist — confirmed via a live Supabase query returning Postgres error `42703 undefined_column`. It has been removed from the list above (see CLAUDE-RULES.md Rule 65). **This one correction does not mean the rest of this document's column lists are reliable.** No table's column list in this file has been independently re-verified against the live schema beyond this single fix. A full audit — every documented column, every table, checked against a live schema query — is a needed future task. Do not assume any other entry here is accurate without checking it directly.
