...
| `action_tokens` | Opaque short-token store backing the match-invite and waitlist-promo signed-link redesign (added July 2026 — replaces the old HMAC-signed long-URL pattern, which was confirmed to get corrupted in transit over real SMS delivery) | `token` (text, PK — short random URL-safe string, e.g. `_iKOQmehS3NN`), `link_type` (text — CHECK constraint `IN ('match_invite','waitlist_promo')`), `payload` (jsonb — arbitrary per-link data resolved server-side, e.g. `{matchId, inviteePhone, inviteeName, organizerEmail}` for `match_invite`, `{matchId, playerEmail}` for `waitlist_promo`), `expiry` (timestamptz), `used_at` (timestamptz, nullable — best-effort observability marker only, never gates re-visiting a still-valid link), `created_at` (timestamptz, default `now()`) |
