...
19. **Waitlist-promotion success screen missing "See Match Details" button.** Tonight's button was added only to `match-invite-respond.js`'s direct-invite success state. Prosper's waitlist-promotion acceptance (after Lindsy cancelled) showed a working "You're in!" confirmation but no See Match Details button. Needs: locate the waitlist-promotion acceptance function/file and confirm whether it's a genuinely separate code path needing the same button added, or shares logic with `match-invite-respond.js` and the button should have appeared.
