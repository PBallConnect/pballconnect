...
_All 71 rules. No trimming. Cross-reference with CLAUDE.md, CLAUDE-SCHEMA.md, CLAUDE-SMS.md._
...
70. **A found security or privacy gap must be proposed as a fix in the same turn it's found — never just flagged and left open.** This applies whether the gap is the direct target of the current task or discovered incidentally while working on something else (e.g. auditing one table's RLS policies surfaces an unrelated open policy on the same or a different table). "I found X, that's a problem" is an incomplete response — it must be paired with "here's the concrete fix" (SQL, code diff, or a clearly-scoped design if the fix needs one) in the same message, ready for approval, not deferred to a future ask. The bar for proactively proposing a fix on a security/privacy finding is lower than for ordinary product or design decisions: the cost of a known, unresolved data-exposure gap sitting in a live production database outweighs the cost of proposing a fix that turns out to be unwanted. This does not change the approval requirement — a proposed fix is never auto-applied, it still needs explicit user approval before touching live data or committing code, same as every other change.

71. **New or modified user-facing overlays/modals/cards must match the app's established light-theme design system — no ad hoc dark backgrounds.** Found Aug 2 2026: `#inviteAcceptOverlay`, `#inviteReciprocateOverlay`, and `#confirmOverlay` all hardcoded a near-black card background (`#0f1f12`) while reusing the app's `--dim` gray (designed for light/white card contexts), producing unreadable body text — the app is otherwise entirely light-themed (white cards, light borders, dark text). See CLAUDE.md's "Design System Reference" section for the actual approved tokens. Any future overlay, modal, toast, or card should default to this light system unless a deliberate, consistent dark-theme pass is made across the whole app at once — never introduce a one-off dark surface for a single screen. Also applies: abbreviations (e.g. "IC" for Inner Circle) must be spelled out on first use within any given screen/card before being used standalone.
